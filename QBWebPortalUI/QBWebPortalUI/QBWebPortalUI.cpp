...
QBUI interface;                                 // interface controls.
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   HWND hWnd = CreateWindowW(szWindowClass, L"QB Web Portal", WS_OVERLAPPEDWINDOW,
    ...
    case WM_COMMAND:
        {
            ...
            case BU_EDITTIME:
                ...
                break;
            case BU_RUNMIN: 
                MessageBox(hWnd, L"Test", L"Run Minimums Popup!", MB_OK);
                break;
            case BU_START:
            {
                ...
                GetWindowText(interface.m_controls.at((int)ctrlS::INTERVALEDIT), c, 128);
...
INT_PTR CALLBACK Connection(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        if (LOWORD(wParam) == IDOK || LOWORD(wParam) == IDCANCEL)
        {
            const int ipLength = 15;
            TCHAR c[ipLength];
            GetDlgItemText(hDlg, IDC_SQLIP, c, ipLength);

            std::wstring x = L"Test String Concat: " + std::wstring(c);

            MessageBox(hDlg, L"Test", x.c_str(), MB_OK);
            EndDialog(hDlg, LOWORD(wParam));
            return (INT_PTR)TRUE;
        }
        break;
